...
class Get
{
 ...
 public:
	...
	Get()
	{
 		...
 		if(str == NULL) {
			Error("CWEB::IN - Allocated Space for Get - String Default.\n"
	...
	bool init(const long max_size = 0)
	{
		...
		if(strcmp(rm, "GET") != 0) {
			Warn("CWEB::IN - REQUEST_METHOD IS NOT GET.\n"
		...
		if(httpg == NULL) {
			Error("CWEB::IN - HTTP REQUEST METHOD GET STRING is NULL.");
		}
		hglen = strlen(httpg);
		
		if(hglen > max_size && max_size > 0)
		{
			Error("CWEB::IN - size of input data recieve by HTTP GET METHOD is bigger"
		...
		if(size <= hglen)
		{
			...
			if(str == NULL) {
				Error("CWEB::IN - Allocated Space for Cookie - String Default.\n"
				"Tried to allocated %d bytes\nerro is %d\nstr erro is \"%s\"",
				size*sizeof(char), errno, strerror(errno));
			}
		}
	...
	get(const char *key)
	{
		...
		Warn("CWEB::IN - Fetch for a no key of HTTP REQUEST METHOD GET.\n"
		"fectch key = \"%s\"\nnumber of keys is %d\n"
	...
	void fill_map()
	{
		...
		for(int i=0; i < hglen; ++i)
		{
			...
			if(cweb::decode(keyDecode, key, keyLen) != true) { // decode the key
				Error("CWEB::IN - Decoding key in HTTP REQUEST METHOD GET.\n"
			...
			if(cweb::decode(contentDecode, content, contentLen) != true) {
				Error("CWEB::IN - Decoding content in HTTP REQUEST METHOD GET.\n"
				"content enconding: \"%s\"\nkey decode (wrong value): \"%s\"\n"
				"content length: %d\nHTTP GET: \"%s\"\n",
